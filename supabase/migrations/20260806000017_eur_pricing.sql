-- Admin-settable EUR prices, used instead of live FX conversion whenever set
-- (fixes odd-looking auto-converted Netherlands prices). Nullable — no value
-- means "keep auto-converting from GHS", so every existing product keeps
-- working exactly as before until an admin opts a product in.
alter table public.products
  add column if not exists eur_regular_price numeric(10, 2) check (eur_regular_price >= 0);

alter table public.products
  add column if not exists eur_sale_price numeric(10, 2) check (eur_sale_price >= 0);

alter table public.products
  drop constraint if exists products_eur_sale_price_check;

alter table public.products
  add constraint products_eur_sale_price_check
    check (eur_sale_price is null or eur_regular_price is null or eur_sale_price <= eur_regular_price);

-- search_products must be dropped first: Postgres won't let create-or-replace
-- change a TABLE-returning function's output columns in place.
