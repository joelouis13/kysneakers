-- Flash Sales: a third admin-set organization tag alongside Featured and New
-- Arrival (lib/admin/products/schemas.ts's productFormSchema), independent of
-- is_on_sale (which drives an actual discounted sale_price). Lets staff flag
-- products for a flash-sale promotion without necessarily discounting them.
